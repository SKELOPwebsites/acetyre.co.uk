<template>
    <div class="border-x border-b rounded-b-lg border-gray-300 font-montserrat">
        ...
        <div class="w-full p-4">
            <template v-for="(timeslot, index) in selectedDate.timeslots">
                <template v-if="timeslot.type === 'anytime'">
                    <TimeslotMob @selectTimeslot="(form, timeslotSelected) => $emit('selectTimeslot', form, timeslotSelected)" :timeslot="timeslot" :date="selectedDate" :key="selectedDate.date + timeslot.from"/>
                </template>
            </template>
        </div>
